...
- **`applgrid-mcfm103-cmake.patch` must contain applgrid hunks ONLY.** An earlier
  revision was generated by diffing pristine 10.3 against a tree that already
  had an (older) aarch64 CMake patch applied, so it silently carried the
  aarch64 hunks as well. On arm64 build.sh then applied `aarch64-cmake.patch`
  on top and all four of its hunks failed (`Hunk #1 FAILED at 63 ... #4 at
  327`) -- which broke plain 10.3 on arm64 too, since these patches are applied
  to every 10.x build. It now carries a single hunk anchored on lines the
  aarch64 patch never touches, and was verified at `--fuzz=0` in both orders:
  pristine + applgrid (x86) and pristine + applgrid + aarch64 (arm64).
- **Only the `mcfm` executable links the bridge.** build.sh passes
  `-Dwith_library=ON`, and `libmcfm` is created further down CMakeLists.txt than
  the applgrid block, so setting a property on it there fails with "could not
  find TARGET libmcfm". It does not need the bridge anyway: gridwrap's stubs
  no-op without it.
